...
def test_soiling_srr(soiling_normalized_daily, soiling_insolation, soiling_times):
    ...
    expected_means = expected_means[
        ["soiling_rate", "soiling_rate_low", "soiling_rate_high", "inferred_start_loss",
    ...
    pd.testing.assert_index_equal(
        soiling_info["soiling_ratio_perfect_clean"].index, soiling_times, check_names=False)
    sr_mean = soiling_info["soiling_ratio_perfect_clean"].mean()
    assert 0.968265 == pytest.approx(
        sr_mean, abs=1e-6
    ), "The mean of soiling_info['soiling_ratio_perfect_clean'] differs from expected"
    assert isinstance(
        soiling_info["soiling_ratio_perfect_clean"], pd.Series
...
def test_soiling_srr_clean_threshold(soiling_normalized_daily, soiling_insolation):
    ...
    with pytest.raises(NoValidIntervalError):
        np.random.seed(1977)
        sr, sr_ci, soiling_info = soiling_srr(
            soiling_normalized_daily, soiling_insolation, reps=10, clean_threshold=0.1)


def test_soiling_srr_trim(soiling_normalized_daily, soiling_insolation):
    np.random.seed(1977)
    sr, sr_ci, soiling_info = soiling_srr(
        soiling_normalized_daily, soiling_insolation, reps=10, trim=True)
...
def test_soiling_srr_min_interval_days(soiling_normalized_daily, soiling_insolation):
    ...
    with pytest.raises(NoValidIntervalError):
        ...
        _ = soiling_srr(soiling_normalized_daily, soiling_insolation, confidence_level=68.2,
                        reps=10, min_interval_days=26)

    # but min=24 should be fine:
    _ = soiling_srr(soiling_normalized_daily, soiling_insolation, confidence_level=68.2,
                    reps=10, min_interval_days=24)


def test_soiling_srr_recenter_false(soiling_normalized_daily, soiling_insolation):
    np.random.seed(1977)
    sr, sr_ci, soiling_info = soiling_srr(
        soiling_normalized_daily, soiling_insolation, reps=10, recenter=False)
...
def test_soiling_srr_negative_step(soiling_normalized_daily, soiling_insolation):
    ...
    assert list(soiling_info["soiling_interval_summary"]["valid"].values) == [
        True, False, True], \
...
def test_soiling_srr_with_nan_interval(soiling_normalized_daily, soiling_insolation):
    ...
    sr, sr_ci, soiling_info = soiling_srr(normalized_corrupt, soiling_insolation,
                                          ...
                                          detect_neg_shifts=True, piecewise_fit=True)
    assert 0.974103 == pytest.approx(sr, abs=1e-6), \
        "Soiling ratio different from expected value when an entire interval was NaN"


def test_soiling_srr_outlier_factor(soiling_normalized_daily, soiling_insolation):
    _, _, info = soiling_srr(
        soiling_normalized_daily, soiling_insolation, reps=1, outlier_factor=8
    )
...
def test_soiling_srr_min_interval_days_default(
    ...
    np.random.seed(1977)
    sr, sr_ci, soiling_info = soiling_srr(
        soiling_normalized_daily[start:], soiling_insolation[start:], reps=reps
    )
    assert expected_sr == pytest.approx(sr, abs=1e-6), \
        "Soiling ratio different from expected value"
...
@pytest.mark.parametrize(
    "test_param", ["energy_normalized_daily", "insolation_daily", "precipitation_daily"])
...
def test_negative_shifts(
    soiling_normalized_daily_with_neg_shifts, soiling_insolation, soiling_times,
        method, detect_neg_shifts, expected_sr):
    reps = 10
    np.random.seed(1977)
    sr, sr_ci, soiling_info = soiling_srr(soiling_normalized_daily_with_neg_shifts,
                                          soiling_insolation, reps=reps,
                                          method=method, detect_neg_shifts=detect_neg_shifts)
    assert expected_sr == pytest.approx(sr, abs=1e-6), \
...
def test_piecewise(soiling_normalized_daily_with_piecewise_slope, soiling_insolation,
                   soiling_times, method, piecewise_fit, expected_sr):
    reps = 10
    np.random.seed(1977)
    ...
    sr, sr_ci, soiling_info = soiling_srr(soiling_normalized_daily_with_piecewise_slope,
                                          soiling_insolation, reps=reps, method=method,
                                          detect_neg_shifts=detect_neg_shifts,
...
def test_piecewise_and_neg_shifts(soiling_normalized_daily_with_piecewise_slope,
    ...
    sr, sr_ci, soiling_info = soiling_srr(soiling_normalized_daily_with_piecewise_slope,
                                          soiling_insolation, reps=reps,
                                          method="perfect_clean", detect_neg_shifts=True,
                                          piecewise_fit=True)
    assert 0.896936 == pytest.approx(sr, abs=1e-6), \
        "Soiling ratio different from expected value for data with piecewise slopes"
    np.random.seed(1977)
    sr, sr_ci, soiling_info = soiling_srr(soiling_normalized_daily_with_neg_shifts,
                                          soiling_insolation, reps=reps,
                                          method="perfect_clean", detect_neg_shifts=True)
    assert 0.964117 == pytest.approx(sr, abs=1e-6), \
        "Soiling ratio different from expected value for data with negative shifts"


def test_detect_neg_shifts_clean_threshold(soiling_normalized_daily_with_neg_shifts,
                                           soiling_insolation):
    """Test that clean_threshold works with detect_neg_shifts=True and
    can cause no soiling intervals to be found"""
    np.random.seed(1977)
    sr, sr_ci, soiling_info = soiling_srr(soiling_normalized_daily_with_neg_shifts,
                                          soiling_insolation, reps=10, clean_threshold=0.1,
                                          method="perfect_clean", detect_neg_shifts=True,
                                          piecewise_fit=True)
    ...
    with pytest.raises(NoValidIntervalError):
        np.random.seed(1977)
        soiling_srr(soiling_normalized_daily_with_neg_shifts,
                    soiling_insolation, reps=10, clean_threshold=1)
...
def test_annual_soiling_ratios(multi_year_profiles):
    expected_data = np.array([[2018, 4.5, 1.431, 7.569], [2019, 14.5, 11.431, 17.569]])
...
def test_annual_soiling_ratios_confidence_interval(multi_year_profiles):
    expected_data = np.array([[2018, 4.5, 0.225, 8.775], [2019, 14.5, 10.225, 18.775]])
    expected = pd.DataFrame(
        data=expected_data,
        columns=["year", "soiling_ratio_median", "soiling_ratio_low", "soiling_ratio_high"])
    expected["year"] = expected["year"].astype(int)
...
def test_monthly_soiling_rates(soiling_interval_summary):
    ...
    expected = np.array(
        [[1.00000000e00, -2.42103810e-03, -5.00912766e-03, -7.68551806e-04, 2.00000000e00],
         [2.00000000e00, -1.25092837e-03, -2.10091842e-03, -3.97354321e-04, 1.00000000e00],
         [3.00000000e00, -2.00313359e-03, -2.68359541e-03, -1.31927678e-03, 1.00000000e00],
         [4.00000000e00, -1.99729563e-03, -2.68067699e-03, -1.31667446e-03, 1.00000000e00]])
...
def test_monthly_soiling_rates_min_interval_days(soiling_interval_summary):
    ...
    expected = np.array(
        [[1.00000000e00, -1.24851539e-03, -2.10394564e-03, -3.98358211e-04, 1.00000000e00],
         [2.00000000e00, -1.25092837e-03, -2.10091842e-03, -3.97330424e-04, 1.00000000e00],
         [3.00000000e00, -2.00309454e-03, -2.68359541e-03, -1.31927678e-03, 1.00000000e00],
         [4.00000000e00, -1.99729563e-03, -2.68067699e-03, -1.31667446e-03, 1.00000000e00]])
    expected = _build_monthly_summary(expected)
...
def test_monthly_soiling_rates_max_slope_err(soiling_interval_summary):
    np.random.seed(1977)
    result = monthly_soiling_rates(
        soiling_interval_summary, max_relative_slope_error=120
    )

    expected = np.array(
        [[1.00000000e00, -4.74910923e-03, -5.26236739e-03, -4.23901493e-03, 1.00000000e00],
         [2.00000000e00, np.nan, np.nan, np.nan, 0.00000000e00],
         [3.00000000e00, -2.00074270e-03, -2.68073474e-03, -1.31786434e-03, 1.00000000e00],
         [4.00000000e00, -2.00309454e-03, -2.68359541e-03, -1.31927678e-03, 1.00000000e00]])
    expected = _build_monthly_summary(expected)
...
def test_monthly_soiling_rates_confidence_level(soiling_interval_summary):
    ...
    expected = np.array(
        [[1.00000000e00, -2.42103810e-03, -5.42313113e-03, -1.21156562e-04, 2.00000000e00],
         [2.00000000e00, -1.25092837e-03, -2.43731574e-03, -6.23842627e-05, 1.00000000e00],
         [3.00000000e00, -2.00313359e-03, -2.94998476e-03, -1.04988760e-03, 1.00000000e00],
         [4.00000000e00, -1.99729563e-03, -2.95063841e-03, -1.04869949e-03, 1.00000000e00]])
...
def test_monthly_soiling_rates_reps(soiling_interval_summary):
    ...
    expected = np.array(
        [[1.00000000e00, -2.88594088e-03, -5.03736679e-03, -6.47391131e-04, 2.00000000e00],
         [2.00000000e00, -1.67359565e-03, -2.00504171e-03, -1.33240044e-03, 1.00000000e00],
         [3.00000000e00, -1.22306993e-03, -2.19274892e-03, -1.11793240e-03, 1.00000000e00],
         [4.00000000e00, -1.94675549e-03, -2.42574164e-03, -1.54850795e-03, 1.00000000e00]])
